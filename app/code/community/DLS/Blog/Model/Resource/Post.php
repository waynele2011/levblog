...
class DLS_Blog_Model_Resource_Post extends Mage_Catalog_Model_Resource_Abstract {

    protected $_postProductTable = null;
    protected $_postTaxonomyTable = null;
    protected $_postTagTable = null;

    public function __construct() {
        $resource = Mage::getSingleton('core/resource');
        $this->setType('dls_blog_post')
                ->setConnection(
                        $resource->getConnection('post_read'), $resource->getConnection('post_write')
        );
        $this->_postProductTable = $this->getTable('dls_blog/post_product');
        $this->_postTaxonomyTable = $this->getTable('dls_blog/post_taxonomy');
        $this->_postTagTable = $this->getTable('dls_blog/post_tag');
    }
    ...
    protected function _initCheckUrlKeySelect($urlKey, $store) {
        $urlRewrite = Mage::getModel('eav/config')->getAttribute('dls_blog_post', 'url_key');
