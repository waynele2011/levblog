...
class DLS_Blog_Model_Resource_Blogset_Collection extends Mage_Core_Model_Resource_Db_Collection_Abstract {
    ...
    public function addTaxonomyFilter($taxonomy) {
        ...
        if (!isset($this->_joinedFields['taxonomy'])) {
            $this->getSelect()->join(
                    array('related_taxonomy' => $this->getTable('dls_blog/blogset_taxonomy')), 'related_taxonomy.blogset_id = main_table.entity_id', array('position')
            );
