...
class DLS_Blog_Model_Resource_Tag_Collection extends Mage_Core_Model_Resource_Db_Collection_Abstract {
    ...
    public function addPostFilter($post) {
        ...
        if (!isset($this->_joinedFields['post'])) {
            $this->getSelect()->join(
                    array('related_post' => $this->getTable('dls_blog/tag_post')), 'related_post.tag_id = main_table.entity_id', array('position')
            );
