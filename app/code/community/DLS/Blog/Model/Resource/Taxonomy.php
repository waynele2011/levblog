...
class DLS_Blog_Model_Resource_Taxonomy extends Mage_Core_Model_Resource_Db_Abstract {

    protected $_tree;

    public function _construct() {
        $this->_init('dls_blog/taxonomy', 'entity_id');
    }

    protected function _getTree() {
        if (!$this->_tree) {
            $this->_tree = Mage::getResourceModel('dls_blog/taxonomy_tree')->load();
        }
        return $this->_tree;
    }
    ...
    public function isForbiddenToDelete($taxonomyId) {
        return ($taxonomyId == Mage::helper('dls_blog/taxonomy')->getRootTaxonomyId());
    }
    ...
    DLS_Blog_Model_Taxonomy $taxonomy, DLS_Blog_Model_Taxonomy $newParent, $afterTaxonomyId = null
    ) {
