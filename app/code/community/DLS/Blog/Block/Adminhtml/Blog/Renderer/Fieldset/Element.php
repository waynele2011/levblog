...
class DLS_Blog_Block_Adminhtml_Blog_Renderer_Fieldset_Element extends Mage_Adminhtml_Block_Widget_Form_Renderer_Fieldset_Element {

    protected function _construct() {
        $this->setTemplate('dls_blog/form/renderer/fieldset/element.phtml');
    }
    ...
    public function getScopeLabel() {
        ...
        if ($this->isScopeGlobal($attribute)) {
            $html.= Mage::helper('dls_blog')->__('[GLOBAL]');
        } elseif ($this->isScopeWebsite($attribute)) {
            $html.= Mage::helper('dls_blog')->__('[WEBSITE]');
        } elseif ($this->isScopeStore($attribute)) {
            $html.= Mage::helper('dls_blog')->__('[STORE VIEW]');
        }
        return $html;
    }
