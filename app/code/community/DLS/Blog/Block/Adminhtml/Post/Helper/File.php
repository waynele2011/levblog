...
class DLS_Blog_Block_Adminhtml_Post_Helper_File extends Varien_Data_Form_Element_Abstract {
    ...
    public function getElementHtml() {
        ...
        if ($this->getValue()) {
            ...
            if (!preg_match("/^http\:\/\/|https\:\/\//", $url)) {
                $url = Mage::helper('dls_blog/post')->getFileBaseUrl() . $url;
            }
    ...
    protected function _getDeleteCheckbox() {
        $html = '';
        if ($this->getValue()) {
            $label = Mage::helper('dls_blog')->__('Delete File');
