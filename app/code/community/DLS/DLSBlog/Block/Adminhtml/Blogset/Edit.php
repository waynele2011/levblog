...
class DLS_DLSBlog_Block_Adminhtml_Blogset_Edit extends Mage_Adminhtml_Block_Widget_Form_Container {
    ...
    public function __construct() {
        ...
        $this->_updateButton(
                'save', 'label', Mage::helper('dls_dlsblog')->__('Save Blog setting')
        );
        $this->_updateButton(
                'delete', 'label', Mage::helper('dls_dlsblog')->__('Delete Blog setting')
        );
        $this->_addButton(
    ...
    public function getHeaderText() {
        if (Mage::registry('current_blogset') && Mage::registry('current_blogset')->getId()) {
            return Mage::helper('dls_dlsblog')->__(
                            "Edit Blog setting '%s'", $this->escapeHtml(Mage::registry('current_blogset')->getName())
            );
        } else {
