...
class DLS_DLSBlog_Block_Adminhtml_Blogset_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form {
    ...
    protected function _prepareForm() {
        ...
        $fieldset = $form->addFieldset(
                'blogset_form', array('legend' => Mage::helper('dls_dlsblog')->__('Blog setting'))
        );

        $fieldset->addField(
                'name', 'text', array(
            'label' => Mage::helper('dls_dlsblog')->__('Blog name'),
            'name' => 'name',
            'required' => true,
        ...
        $valuesFilter = Mage::getResourceModel('dls_dlsblog/filter_collection')
                ->toOptionArray();
        array_unshift($valuesFilter, array('label' => '', 'value' => ''));
        $html = '<a href="{#url}" id="blogset_filter_id_link" target="_blank"></a>';
        $html .= '<script type="text/javascript">
            function changeLayoutdesignIdLink() {
                if ($(\'blogset_filter_id\').value == \'\') {
                    $(\'blogset_filter_id_link\').hide();
                } else {
                    $(\'blogset_filter_id_link\').show();
                    var url = \'' . $this->getUrl('adminhtml/dlsblog_filter/edit', array('id' => '{#id}', 'clear' => 1)) . '\';
                    var text = \'' . Mage::helper('core')->escapeHtml($this->__('View {#name}')) . '\';
                    var realUrl = url.replace(\'{#id}\', $(\'blogset_filter_id\').value);
                    $(\'blogset_filter_id_link\').href = realUrl;
                    $(\'blogset_filter_id_link\').innerHTML = text.replace(\'{#name}\', $(\'blogset_filter_id\').options[$(\'blogset_filter_id\').selectedIndex].innerHTML);
                }
            }
            $(\'blogset_filter_id\').observe(\'change\', changeLayoutdesignIdLink);
            changeLayoutdesignIdLink();
            </script>';
        $fieldset->addField(
                'filter_id', 'select', array(
            'label' => Mage::helper('dls_dlsblog')->__('Main filter'),
            'name' => 'filter_id',
            'required' => false,
            'values' => $valuesFilter,
            'after_element_html' => $html
