...
class DLS_DLSBlog_Block_Adminhtml_Filter_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    public function __construct()
    {
        ...
        $this->_updateButton(
            'save',
            'label',
            Mage::helper('dls_dlsblog')->__('Save Filter')
        );
        $this->_updateButton(
            'delete',
            'label',
            Mage::helper('dls_dlsblog')->__('Delete Filter')
        );
        $this->_addButton(
            'saveandcontinue',
            array(
    ...
    public function getHeaderText()
    {
        if (Mage::registry('current_filter') && Mage::registry('current_filter')->getId()) {
            return Mage::helper('dls_dlsblog')->__(
                "Edit Filter '%s'",
                $this->escapeHtml(Mage::registry('current_filter')->getName())
            );
        } else {
