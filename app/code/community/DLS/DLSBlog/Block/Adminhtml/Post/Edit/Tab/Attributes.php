...
class DLS_DLSBlog_Block_Adminhtml_Post_Edit_Tab_Attributes extends Mage_Adminhtml_Block_Widget_Form {
    ...
    protected function _getAdditionalElementHtml($element) {
        ...
        if ($element->getName() == 'post_tag') {
            $html = '<input id="post_tag_add" value="" class=" input-text" type="text">'
                    . '<div id="tag-manager">'
                    . '<button type="button"><span><span><span>Add tag</span></span></span></button>'
                    . '<div id="list-tags"></div>'
                    . '</div>';
            $html .= '<script type="text/javascript">
                    function addTag() {
                        var tag_add = $(\'post_tag_add\').value;
                        var tag_add_arr = tag_add.split(",");
                        var tag = $(\'post_tag\').value;
                        var tag_arr = tag.split(",");
                        console.log(tag_arr.length);
                        tag_add_arr.forEach(function(item, index) {
                            var item = item.trim();
                            if (item != "") {
                                if (tag_arr.indexOf(item) == -1) {
                                    tag_arr.push(item);
                                }
                            }
                        });
                        tag_arr = tag_arr.filter(String);
                        $(\'post_tag\').value = tag_arr.join();
                        initTag();
                        $(\'post_tag_add\').value = "";
                    }
                    function initTag() {
                        ...
                        var tag_arr = tag.split(",");
                        var html = "<ul>";
                        tag_arr.forEach(function(item, index) {
                            html += "<li>"+item+"</li>"
                        });
                        html += "</ul>";
                        var listTags = document.getElementById("list-tags");
                        listTags.innerHTML = "";
                        $$("#list-tags").first().insert(html);
                    }
                    $(\'tag-manager\').observe(\'click\', addTag);
                    $(\'post_tag\').hide();
                    initTag();
                    </script>';
            return $html;
