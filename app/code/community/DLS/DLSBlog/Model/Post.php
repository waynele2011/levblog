...
class DLS_DLSBlog_Model_Post extends Mage_Catalog_Model_Abstract {
    ...
    public function getPostByFilter($id)
    {
        $page = Mage::app()->getRequest()->getParam('p');
        $limit = Mage::app()->getRequest()->getParam('limit');
        $filterCollection = Mage::getResourceModel('dls_dlsblog/filter_collection')
                ->addFieldToFilter('entity_id', $id);
        $filter = $filterCollection->getFirstItem()->getTaxonomyId();
        $postCollection = Mage::getResourceModel('dls_dlsblog/post_collection')
                ->addAttributeToSelect('*')
                ->addAttributeToFilter('status', 1)
                ->addAttributeToFilter('taxonomy_id', $filter)
                ->setPageSize($limit)
                ->setCurPage($page);
