...
/**
 * Layout design tree resource model
 *
 * @category    DLS
 * @package     DLS_DLSBlog
 * @author      Ultimate Module Creator
 */
class DLS_DLSBlog_Model_Resource_Layoutdesign_Tree extends Varien_Data_Tree_Dbp
{
    ...
    /**
     * Layout designs resource collection
     *
     * @var DLS_DLSBlog_Model_Resource_Layoutdesign_Collection
     */
    protected $_collection;
    protected $_storeId;

    /**
     * Inactive layout designs ids
     * @var array
     */

    protected $_inactiveLayoutdesignIds  = null;
    ...
    public function __construct()
    {
        ...
        parent::__construct(
            ...
            $resource->getTableName('dls_dlsblog/layoutdesign'),
            array(
     ...
     *
     * @access public
     * @param DLS_DLSBlog_Model_Resource_Layoutdesign_Collection $collection
     ...
     *
     * @access public
     * @param unknown_type $layoutdesign
     * @param unknown_type $newParent
     * @param unknown_type $prevNode
     * @author Ultimate Module Creator
     */
    public function move($layoutdesign, $newParent, $prevNode = null)
    {
        Mage::getResourceSingleton('dls_dlsblog/layoutdesign')
            ->move($layoutdesign->getId(), $newParent->getId());
        parent::move($layoutdesign, $newParent, $prevNode);
        $this->_afterMove($layoutdesign, $newParent, $prevNode);
    }
     ...
     * @return DLS_DLSBlog_Model_Resource_Layoutdesign_Tree
     */
    protected function _afterMove($layoutdesign, $newParent, $prevNode)
    {
        Mage::app()->cleanCache(array(DLS_DLSBlog_Model_Layoutdesign::CACHE_TAG));
        return $this;
    }

    /**
     * Load whole layout design tree, that will include specified layout designs ids.
     *
     * @access public
     * @param array $ids
    ...
    /**
     * Load array of layout design parents
     *
     * @access public
     * @param string $path
     * @param bool $addCollectionData
    ...
    /**
     * Obtain select for layout designs
    ...
    protected function _createCollectionDataSelect($sorted = true)
    {
        ...
        $layoutdesignsTable = Mage::getSingleton('core/resource')
            ->getTableName('dls_dlsblog/layoutdesign');
        $subConcat = $this->_conn->getConcatSql(array('main_table.path', $this->_conn->quote('/%')));
        $subSelect = $this->_conn->select()
            ->from(array('see' => $layoutdesignsTable), null)
    ...
    /**
     * Get real existing layout design ids by specified ids
     *
     * @access public
     * @param array $ids
     * @return array
     * @author Ultimate Module Creator
     */
    public function getExistingLayoutdesignIdsBySpecifiedIds($ids)
    {
        if (empty($ids)) {
            return array();
        }
    ...
    )
    {
        ...
        if ($toLoad) {
            $collection->load();
            foreach ($collection as $layoutdesign) {
                if ($this->getNodeById($layoutdesign->getId())) {
                    $this->getNodeById($layoutdesign->getId())->addData($layoutdesign->getData());
                }
            }
            foreach ($this->getNodes() as $node) {
    ...
    /**
     * Add inactive layout designs ids
     *
     * @access public
     ...
     * @author Ultimate Module Creator
     */
    public function addInactiveLayoutdesignIds($ids)
    {
        if (!is_array($this->_inactiveLayoutdesignIds)) {
            $this->_initInactiveLayoutdesignIds();
        }
        $this->_inactiveLayoutdesignIds = array_merge($ids, $this->_inactiveLayoutdesignIds);
        return $this;
    }

    /**
     * Retrieve inactive layout designs ids
     *
     * @access protected
     * @return DLS_DLSBlog_Model_Resource_Layoutdesign_Tree
     * @author Ultimate Module Creator
     */
    protected function _initInactiveLayoutdesignIds()
    {
        $this->_inactiveLayoutdesignIds = array();
        return $this;
    }
    /**
     * Retrieve inactive layout designs ids
     ...
     * @author Ultimate Module Creator
     */
    public function getInactiveLayoutdesignIds()
    {
        if (!is_array($this->_inactiveLayoutdesignIds)) {
            $this->_initInactiveLayoutdesignIds();
        }
        return $this->_inactiveLayoutdesignIds;
    }

    /**
     * Return disable layout design ids
     *
     * @access protected
     * @param DLS_DLSBlog_Model_Resource_Layoutdesign_Collection $collection
     * @return array
    ...
    {
        $this->_inactiveItems = $this->getInactiveLayoutdesignIds();
    ...
    /**
     * Retrieve inactive layout design item ids
     *
     * @access protecte
     * @param DLS_DLSBlog_Model_Resource_Layoutdesign_Collection $collection
     * @return array
    ...
    protected function _getInactiveItemIds($collection)
    {
        ...
        $table = Mage::getSingleton('core/resource')->getTable('dls_dlsblog/layoutdesign');
        $bind = array(
            'cond' => 0,
        );
        $select = $this->_conn->select()
    ...
    /**
     * Check is layout design items active
     *
     * @access protecte
     * @param int $id
     * @return boolean
