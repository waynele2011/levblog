...
class DLS_DLSBlog_Adminhtml_Dlsblog_TaxonomyController extends DLS_DLSBlog_Controller_Adminhtml_DLSBlog {
    ...
    public function editAction() {
        ...
        $this->_addBreadcrumb(
                Mage::helper('dls_dlsblog')->__('Manage Taxonomies'), Mage::helper('dls_dlsblog')->__('Manage Taxonomies')
        );
    ...
    public function taxonomiesJsonAction() {
        ...
        if ($taxonomyId = (int) $this->getRequest()->getPost('id')) {
            ...
            $this->getResponse()->setBody(
                    $this->getLayout()->createBlock('dls_dlsblog/adminhtml_taxonomy_tree')
    ...
    public function moveAction() {
        ...
        if (!$taxonomy) {
            $this->getResponse()->setBody(
                    Mage::helper('dls_dlsblog')->__('Taxonomy move error')
            );
            return;
        }
    ...
    public function treeAction() {
        $taxonomyId = (int) $this->getRequest()->getParam('id');
        $taxonomy = $this->_initTaxonomy();
        $block = $this->getLayout()->createBlock('dls_dlsblog/adminhtml_taxonomy_tree');
        $root = $block->getRoot();
        $this->getResponse()->setBody(
        ...
        if ($id = (int) $this->getRequest()->getParam('id')) {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                        Mage::helper('dls_dlsblog')->__('The taxonomy has been deleted.')
                );
            } catch (Mage_Core_Exception $e) {
                ...
                Mage::getSingleton('adminhtml/session')->addError(
                        Mage::helper('dls_dlsblog')->__('An error occurred while trying to delete the taxonomy.')
                );
                $this->getResponse()->setRedirect($this->getUrl('*/*/edit', array('_current' => true)));
                Mage::logException($e);
                return;
    ...
    protected function _isAllowed() {
        return Mage::getSingleton('admin/session')->isAllowed('dls_dlsblog/taxonomy');
    }
    ...
    public function saveAction() {
        ...
        if ($data = $this->getRequest()->getPost('taxonomy')) {
            ...
            $smallImageName = $this->_uploadAndGetName(
                    'small_image', Mage::helper('dls_dlsblog/taxonomy_image')->getImageBaseDir(), $data
            );
            $taxonomy->setData('small_image', $smallImageName);
            $largeImageName = $this->_uploadAndGetName(
                    'large_image', Mage::helper('dls_dlsblog/taxonomy_image')->getImageBaseDir(), $data
            );
            ...
            if (!$taxonomy->getId()) {
                ...
                $parentTaxonomy = Mage::getModel('dls_dlsblog/taxonomy')->load($parentId);
            ...
            try {
                ...
                $taxonomy->save();
                Mage::getSingleton('adminhtml/session')->addSuccess(
                        Mage::helper('dls_dlsblog')->__('The taxonomy has been saved.')
                );
                $refreshTree = 'true';
            } catch (Exception $e) {
