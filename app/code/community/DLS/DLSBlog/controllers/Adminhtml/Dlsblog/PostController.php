...
class DLS_DLSBlog_Adminhtml_Dlsblog_PostController extends Mage_Adminhtml_Controller_Action {
    ...
    public function editAction() {
        ...
        $this->_title($post->getTitile());
        Mage::dispatchEvent(
                'dls_dlsblog_post_edit_action', array('post' => $post)
        );
        $this->loadLayout();
        if ($post->getId()) {
    ...
    public function saveAction() {
        ...
        if ($data) {
            ...
            if (isset($data['filters'])) {
                $post->setFiltersData(
                        Mage::helper('adminhtml/js')->decodeGridSerializedInput($data['filters'])
                );
            }
//            if (isset($postData['post_tag'])) {
//                $post_tags = explode(',',$postData['post_tag']);
//                $tags = array();
//                if (is_array($post_tags)) {
//                    foreach ($post_tags as $_post_tag) {
//                        $tag_collection = Mage::getModel('dls_dlsblog/tag')->getCollection()
//                            ->addFieldToFilter('name', array('eq' => $_post_tag))
//                            ->addFieldToSelect('name');
//                        if ($tag_collection->getSize() > 0) {
//                            foreach ($tag_collection as $_tag) {
//                                $tags[$_tag->getId()] = array('position' => 0);
//                            }
//                        } else {
//                            $tag = Mage::getModel('dls_dlsblog/tag');
//                            $tag->setSlug(Mage::helper('dls_dlsblog')->sanitizeStringForUrl($_post_tag));
//                            $tag->setName($_post_tag);
//                            $tag->save();   
//                            $tags[$tag->getId()] = array('position' => 0);
//                        }
//                    }
//                    var_dump($tags); exit();
//                    if (count($tags) > 0) {
...
//            }
        ...
        if (!is_array($postIds)) {
            ...
            try {
                foreach ($postIds as $postId) {
                    ...
                    Mage::dispatchEvent(
                            'dls_dlsblog_controller_post_delete', array('post' => $post)
                    );
                    $post->delete();
                }
                $this->_getSession()->addSuccess(
    ...
    public function wysiwygAction() {
        ...
        $content = $this->getLayout()->createBlock(
                'dls_dlsblog/adminhtml_dlsblog_helper_form_wysiwyg_content', '', array(
    ...
    public function massPublishStatusAction() {
        ...
        try {
            ...
            $this->_getSession()->addException(
                    $e, Mage::helper('dls_dlsblog')->__('An error occurred while updating the posts.')
            );
        }
        $this->_redirect('*/*/', array('store' => $storeId));
    }
    ...
    public function filtersAction() {
        $this->_initPost();
        $this->loadLayout();
        $this->getLayout()->getBlock('dls_dlsblog.post.edit.tab.filter')
                ->setPostFilters($this->getRequest()->getPost('filters', null));
        $this->renderLayout();
    }
